        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (order.order_status !== "pending" &&
                    ...
                    return res.status(400).json({
                        success: false,
                        message: "Voucher can only be applied to pending or awaiting payment orders",
                    });
                }
                // Check store-specific voucher conditions
                if (voucher.discount.store_id &&
                ...
                if (voucher.discount.product_id) {
                    const hasProduct = order.OrderItem.some((item) => item.product_id === voucher.discount.product_id);
                    if (!hasProduct) {
                        return res.status(400).json({
                            success: false,
                ...
                if (voucher.discount.minimum_order &&
                ...
                const updatedVoucher = yield prisma.voucher.update({
